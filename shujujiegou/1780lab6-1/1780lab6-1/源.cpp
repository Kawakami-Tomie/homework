...
bool reasonable(int t1,int t2,int t3,int num1,int num2,int num3)
{
	...
	else if(t1<t2||t1<t3)//和BC玩家出牌的下标进行比较
	{
		cout<<"出的牌小了！"<<endl;
		return false;
	}
	else if(num2>1||num3>1)//BC玩家出的不是单张
		cout<<"出的牌小了！"<<endl;
	else
		return true;
		}
		...
		if(t1>0||t3>0)//AC已出牌
		{
			if(t1>=t3)//A大
			{
				...
				else //单张没有更大的
				{
					...
					//}
					//else
				    if(minZhadan(list)!=NULL)//没有对子就看炸弹
					{	
							p=minZhadan(list);
							t2=list.Data(p);
							x=p->succ;
...
bool unlimit(int &n,int num2,int num3,int &t1,int t2,int t3,List<int>&list)
{
	...
	if(A.find(t1)!=NULL && A.find(c)!=NULL){//输入都合法，再进行查重
		...
		//合法再判断
		else if(n==2 && num2==2 && num3==1 && t1<t2)//C过，与B比较
		{	cout<<"出的牌太小了！"<<endl;
		return false;}
		else if(n==2 && num2==1 && num3==2 && t1<t3)//B过，与C比较
		{	cout<<"出的牌太小了！"<<endl;
		return false;}
		else if(n==2 && num2==2 && num3==2 && (t1<t3||t1<t2))//都是出对子，若A小于BC中任何一个
		{	cout<<"出的牌太小了！"<<endl;
		return false;}
		else if(n==4 && num2==4 && num3!=4 && t1<t2)//出炸弹，小概率
		{	cout<<"出的牌太小了！"<<endl;
		return false;}
		...
		return false;}
			...
			{
				deletePoker(52, A);
				stack.insertAsLast(52);
				deletePoker(53, A);
				stack.insertAsLast(53);
				return true;
			}
...
int main()
{
	...
	while(!((A.empty()&&B.empty())||(C.empty()&&B.empty())||(A.empty()&&C.empty()))){//至少有两个列表非空
		...
		if(!A.empty()){//A列表没有清空时执行
			...
			}while(!reasonable(t1,t2,t3, pok_num1, pok_num2, pok_num3));//判断合法性
			if(pok_num1 ==1)
			{
				if(pok!="过")//出得起
					{	
				...
				while(!unlimit(pok_num1, pok_num2, pok_num3,t1,t2,t3,A));
		...
		if(!B.empty()){
			cout<<"玩家B出牌："<<endl;
			t2=chuPai(t1,t2,t3, pok_num1, pok_num2, pok_num3,B);
				if(B.empty())
				{	
					p2=B.Place(place);
					place++;
		...
		if(!C.empty()){
			cout<<"玩家C出牌："<<endl;
			t3=chuPai(t1,t3,t2, pok_num1, pok_num3, pok_num2,C);//注意顺序
			if(C.empty())
			{	
				p3=C.Place(place);
				place++;
