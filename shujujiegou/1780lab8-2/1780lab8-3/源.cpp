...
Queue<int> setoffTrack,SetoffTrack;
int buffer0, buffer1, buffer2, buffer3;
int Buff1,Buff2, Buff3, Buff4,Buff5,buff1,buff2,buff3,buff4,buff5;
void findtrack()
{
...
again_2arrival:
    vector<int> Arrival1{ 6,10,4,8,9,7,1,5,2,3 };
    random_device rd1;
    mt19937 g1(rd1());
    ...
    for (int i = 0; i < 4; i++)//重新生成缓冲序列时先清空
    {
        if (!bufferTracks[i].empty())
        {
            int Tracklength = bufferTracks[i].Size();
            for (int i = 0; i < Tracklength; i++) {
                bufferTracks[i].dequeue();
            }
        }
    }
    ...
    bool find_flag = 1;//查找标志
    int BufferTrack = -1;
    int Canpushtosetofftrack = -1;
    while (!(arrival1.empty() && arrival2.empty() && bufferTracks[0].empty() && bufferTracks[1].empty() && bufferTracks[2].empty() && bufferTracks[3].empty()))//到达序列非空执行
    {
        ...
        if (!(arrival1.empty()) && !(arrival2.empty())) {//到达队列同时非空
            if (bufferTracks[BufferTrack].empty()) {
                if (arrival1.front() < arrival2.front()) {
                    arrival1.enqueue_front_to_rear(bufferTracks[BufferTrack]);
                    cout << arrival1.front() << "号车厢推入" << BufferTrack + 1 << "#缓冲轨" << endl;
                    cout << "缓冲轨" << BufferTrack + 1 << "此时序列：";
                    bufferTracks[BufferTrack].display();
                    arrival1.dequeue();
                }
                else if (arrival1.front() > arrival2.front()) {
                    arrival2.enqueue_front_to_rear(bufferTracks[BufferTrack]);
                    cout << arrival2.front() << "号车厢推入" << BufferTrack + 1 << "#缓冲轨" << endl;
                    cout << "缓冲轨" << BufferTrack + 1 << "此时序列：";
                    bufferTracks[BufferTrack].display();
                    arrival2.dequeue();
            ...
            else if (!bufferTracks[BufferTrack].empty()) {
                if (arrival1.front() < arrival2.front()) {
                    if (arrival1.front() > bufferTracks[BufferTrack].peek()) {
                        arrival1.enqueue_front_to_rear(bufferTracks[BufferTrack]);
                        cout << arrival1.front() << "号车厢推入" << BufferTrack + 1 << "#缓冲轨" << endl;
                        cout << "缓冲轨" << BufferTrack + 1 << "此时序列：";
                        bufferTracks[BufferTrack].display();
                    ...
                    else if (arrival1.front() < bufferTracks[BufferTrack].peek()) {
                        if (arrival2.front() > bufferTracks[BufferTrack].peek()) {
                            arrival2.enqueue_front_to_rear(bufferTracks[BufferTrack]);
                            cout << arrival2.front() << "号车厢推入" << BufferTrack + 1 << "#缓冲轨" << endl;
                            cout << "缓冲轨" << BufferTrack + 1 << "此时序列：";
                            bufferTracks[BufferTrack].display();
                        ...
                        else   if (BufferTrack == 3) {
                            cout << "无法编组，是否重试（Y/N）" << endl;
                            cin >> choose;
                            if (choose == 'Y') {
                                goto again_2arrival;
                            }
                    ...
                    else if (arrival2.front() < bufferTracks[BufferTrack].peek()) {
                        if (arrival1.front() > bufferTracks[BufferTrack].peek()) {
                            arrival1.enqueue_front_to_rear(bufferTracks[BufferTrack]);
                            cout << arrival1.front() << "号车厢推入" << BufferTrack + 1 << "#缓冲轨" << endl;
                            cout << "缓冲轨" << BufferTrack + 1 << "此时序列：";
        ...
        if (!(arrival1.empty()) && arrival2.empty()) {//到达1非空，到达2空
            if (bufferTracks[BufferTrack].empty()) { 
                    arrival1.enqueue_front_to_rear(bufferTracks[BufferTrack]);
                    cout << arrival1.front() << "号车厢推入" << BufferTrack + 1 << "#缓冲轨" << endl;
                    cout << "缓冲轨" << BufferTrack + 1 << "此时序列：";
        ...
        if (!(arrival2.empty()) && arrival1.empty()) {//到达2非空，到达1空
            if (bufferTracks[BufferTrack].empty()) {
                arrival2.enqueue_front_to_rear(bufferTracks[BufferTrack]);
                cout << arrival2.front() << "号车厢推入" << BufferTrack + 1 << "#缓冲轨" << endl;
                arrival2.dequeue();
        ...
        if (arrival2.empty() && arrival1.empty()) {//到达序列清除完毕
            Canpushtosetofftrack = min_track();//缓冲轨道自我比较找最小
            bufferTracks[Canpushtosetofftrack].enqueue_front_to_rear(SetoffTrack);;
            // 打印编组动作
            cout << bufferTracks[Canpushtosetofftrack].front() << "号车厢从" << Canpushtosetofftrack + 1 << "#缓冲轨推入出发区，出发队列为";
    ...
    cout << "是否重新编组（Y/N）" << endl;
    cin >> choose;
    if (choose == 'Y')goto again_2arrival;
