...
public:
	...
	{
		m_score =( m_dailyscore + m_endscore)*0.5;
	}
	int m_id;
	string m_name;
	double m_dailyscore;//平时成绩
	double m_endscore;//期末成绩
	double m_score;//总成绩
};
Student maxscore(vector<Student> xarr_student)//统计最高分
{
	double maxdailyscore = 0; double maxendscore = 0; double maxscore = 0;
	...
	Student temp;
	temp.m_dailyscore = maxdailyscore;//平时成绩的最高分
...
{
	double mindailyscore = 100; double minendscore = 100; double minscore = 100;
	for (int i = 0; i < xarr_student.size(); i++)
	{
...
{
	double avedailyscore = 0; double aveendscore = 0; double avescore = 0;
	for (int i = 0; i < xarr_student.size(); i++)
	{
...
void dailyscorenum(vector<Student> xarr_student)//平时分分数段
{
	...
	while (1)
	{
		...
		if (identchoice == 1) //老师身份
		{
			...
			if (ifs_sub.is_open())
			{
				Subject tempsub;
				arr_subject.clear();
				while (ifs_sub >> tempsub.m_dailycount >> tempsub.m_endcount)
				{
			...
			cin >> teacherchoice;
			cout << endl;
			int m_dailycount; int m_endcount;
			if (teacherchoice == 1) //进入课程信息
			{
				while (1) //在vector中存储或修改课程信息
				{
					int subchoice = 0; string temp_subjectproject = "";
					cout << endl;
					...
					if (subchoice == 1)
					{
						...
						{
						cout << "请输入课程的评分项目、占比（不用输入%）：" << endl;
						cin >> subject.m_subjectproject; cin >> subject.m_subjectcount;
					...
					else if (subchoice == 2)
					{
						...
						{
								bool subcheck = 1;
								cout << "请输入要修改课程的评分项目" << endl;
								cin >> temp_subjectproject;
								for (int i = 0; i < arr_subject.size(); i++)
								{
									if (arr_subject[i].m_subjectproject == temp_subjectproject)
									{
										cout << "请输入您要将评分项目项目修改为：" << endl;
										cin >> arr_subject[i].m_subjectproject;
										cout << "请输入您要将占比（不用输入%）修改为：" << endl;
										cin >> arr_subject[i].m_subjectcount;
										subcheck = 0;
									}
								}
								if (subcheck)
				...
				{
					ofs_sub << arr_subject[i].m_subjectproject << " " << arr_subject[i].m_subjectcount << endl;
				}
				ofs_sub.close();
			}
			else if (teacherchoice == 2) //进入学生信息
			{
				while (1)
				{
					...
					case 1: //录入学生成绩
						...
						cout << "请输入学生实验成绩："; cin >> student.m_dailyscore;
						cout << "请输入学生作业成绩："; cin >> student.m_endscore;
						cout << endl;
						student.cul_score();
					...
					case 2: //修改学生成绩
						...
						for (int i = 0; i < arr_student.size(); i++)
						{
							...
							{
								cout << "请输入学生实验成绩修改为："; cin >> arr_student[i].m_dailyscore;
								cout << "请输入学生作业成绩修改为："; cin >> arr_student[i].m_endscore;
				...
				{
					ofs_stu << arr_student[i].m_id << " " << arr_student[i].m_name << " " << arr_student[i].m_dailyscore << " " << arr_student[i].m_endscore << " " << arr_student[i].m_score << endl;
				}
				ofs_stu.close();
			}
			else if (teacherchoice == 3) //信息显示
			{
				...
				if (showchoice == 1)
				{
					...
					else
					{
						...
						for (int i = 0; i < arr_subject.size(); i++)
						{
							cout << arr_subject[i].m_subjectproject << "\t" << arr_subject[i].m_subjectcount << endl << endl;
						}
					}
				}
				else if (showchoice == 2)
				{
					...
					{
						cout << "学号\t姓名\t作业\t实验\t总成绩" << endl;
						for (int i = 0; i < arr_student.size(); i++)
						{
							cout << arr_student[i].m_id << "\t" << arr_student[i].m_name << "\t" << arr_student[i].m_dailyscore << "\t" << arr_student[i].m_endscore << "\t" << arr_student[i].m_score << endl << endl;
						}
					}
				}
				else if (showchoice == 3)
				{
					...
					else
					{
						...
						cout << "作业成绩" << maxscore(arr_student).m_dailyscore << "\t" << minscore(arr_student).m_dailyscore << "\t" << avescore(arr_student).m_dailyscore << "\t"; dailyscorenum(arr_student);
						cout << "实验成绩" << maxscore(arr_student).m_endscore << "\t" << minscore(arr_student).m_endscore << "\t" << avescore(arr_student).m_endscore << "\t"; endscorenum(arr_student);
						cout << endl;
					}
				}
			}
		...
		else if (identchoice == 2) //学生身份
		{
			...
			case 3:	//显示成绩统计
				...
				else
				{
					...
					cout << "实验成绩" << maxscore(arr_student).m_dailyscore << "\t" << minscore(arr_student).m_dailyscore << "\t" << avescore(arr_student).m_dailyscore << "\t"; dailyscorenum(arr_student); cout << endl;
					cout << "作业成绩" << maxscore(arr_student).m_endscore << "\t" << minscore(arr_student).m_endscore << "\t" << avescore(arr_student).m_endscore << "\t"; endscorenum(arr_student); cout << endl;
					break;
				}
				...
				if (findchoice == 1)
				{
					...
					else
					{
						for (int i = 0; i < arr_student.size(); i++)
						{
							...
							{
								cout << "您的成绩为如下" << endl;
								cout << "总成绩\t实验\t作业\t排名\t" << endl;
								cout << arr_student[i].m_score << "\t" << arr_student[i].m_dailyscore << "\t" << arr_student[i].m_endscore << "\t" << ranknum(arr_student[i], arr_student) << endl << endl;
								nofind = 0;
							}
						}
						if (nofind)
					...
					else
					{
						for (int i = 0; i < arr_student.size(); i++)
						{
							...
							if (arr_student[i].m_name == temp_name)
							{
								cout << "您的成绩为如下" << endl;
								cout << "总成绩\t实验\t作业\t排名\t" << endl;
								cout << arr_student[i].m_score << "\t" << arr_student[i].m_dailyscore << "\t" << arr_student[i].m_endscore << "\t" << ranknum(arr_student[i], arr_student) << endl;
								nofind = 0;
							}
						}
						if (nofind)
